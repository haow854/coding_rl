...
async def evaluate_coder(planner, coder, s, sem):
    try:
...
async def coder_evaluation(engine, tokenizer, coder_model):
    # dataset
    scenarios = load_test()
    total = len(scenarios)
    ...
    tasks = [evaluate_coder(planner_fixed, coder, s,sem) for s in scenarios]
