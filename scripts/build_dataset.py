...
passes its own tests under our judge, and save the normalised Problems to JSONL.
    ...
    # on AutoDL, on the full HF dataset (streams; needs `datasets`)
...
from rlcoder.data.load import load_hf, load_jsonl   # noqa: E402
from rlcoder.data.verify import filter_verifiable    # noqa: E402


async def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--source", choices=["jsonl", "hf"], default="jsonl")
    ...
    if a.source == "jsonl":
        ...
        problems = load_hf(limit=a.limit, max_tests=a.max_tests)
